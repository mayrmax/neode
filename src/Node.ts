 ...
 * Node Container
 */
export class Node<T = any> extends Entity<T> {
     ...
     * @return {Node}
     */
    constructor(neode: Neode, model: Model, identity: neo4j.Integer, labels: string[], properties?: Map<string, Property>, eager?: Map<string, string>) {
        super();
        ...
        this._labels = labels;
        this._properties = properties || new Map<string, Property>();
        this._eager = eager || new Map<string, string>();
        this._deleted = false;
    }
    ...
    toJson() {
        ...
        // Eager Promises
        return Promise.all( this._model.eager().map((rel: RelationshipType) => {
            const key = rel.name();
