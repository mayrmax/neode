...
export class Factory {
     ...
     * @return {Node}
     */
    hydrateFirst<T= any>(res: QueryResult, alias: string, definition?): Node<T> | boolean {
    ...
    hydrate(res: QueryResult, alias, definition) {
        if ( !res ) {
            return false;
        }
     ...
     * @return {Node}
     */
    hydrateNode(record, definition?: Model | string | undefined) {
        ...
        // Get Internals
        const identity: neo4j.Integer = record[ EAGER_ID ];
        const labels: string[] = record[ EAGER_LABELS ];
